...
// Helper function to make links work even without https://
...
export function Projects() {
  ...
  // Merge static projects with admin saved links
  const mergedProjects: Project[] = projects.map((p) => {
    if (adminLinks[p.title]) {
      return { ...p, link: adminLinks[p.title] };
    }
    return p;
  });

  // Find the featured project (Prime Land)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-text-secondary max-w-2xl">
            Showcase of modern websites and digital experiences across different industries.
          </p>
        </motion.div>
        ...
        {featured && (
          ...
          >
            <div className="grid lg:grid-cols-2">
              <div className="relative aspect-square lg:aspect-auto lg:h-full overflow-hidden bg-gradient-to-br from-navy-700 to-navy-900 flex items-center justify-center">
                <Eye className="w-20 h-20 text-white/10" />
                <img
                  src={featured.image}
                  alt={featured.title}
                  className="relative z-10 object-cover w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  onError={(e) => (e.currentTarget.style.display = "none")}
                />
                <div className="absolute inset-0 bg-navy-950/30 group-hover:bg-navy-950/0 transition-colors duration-500" />
                
                {featured.demo && (
                  <span className="absolute top-6 left-6 z-20 bg-amber-500/90 text-navy-950 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                    Demo
                  </span>
                )}
              </div>
              ...
              <div className="p-8 lg:p-12 flex flex-col justify-center">
                <span className="text-accent-blue text-sm font-medium mb-2">
                  {featured.category}
                </span>
                ...
                {featured.link ? (
                  <a
                    href={formatLink(featured.link)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-white font-medium hover:text-accent-blue transition-colors group/btn"
                  >
                    Live Demo
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {otherProjects.map((project, index) => (
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-navy-700 to-navy-900 flex items-center justify-center">
